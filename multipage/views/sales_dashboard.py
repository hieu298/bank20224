...
st.markdown("-----")
options = ["All", "VPB", "VIB", "VCB","VBB","VAB","TPB","TCB","STB","SBB","SHB","SGB","PGB","OCB","NVB","MSB","MBB","LPB","HDB"
...
df_sheet1 = pd.read_excel('asset/Bankdata.xlsx', sheet_name=selected_option)
...
for keyword in keywords:
    index = df_sheet1[df_sheet1.iloc[:, 0].str.contains(keyword, na=False)].index
    if len(index) > 0:
        tai_san[keyword] = df_sheet1.iloc[index[0], 1:].dropna().values/1000000000

# Chuyển dữ liệu thành DataFrame
combined_df = pd.DataFrame(tai_san, index=df_sheet1.columns[1:len(list(tai_san.values())[0])+1])
...
# Cập nhật layout của biểu đồ
fig10.update_layout(
    title='Biểu đồ cột chồng về cơ cấu tài sản',
    xaxis_title='Thời gian',
    yaxis_title='Giá trị  (Tỷ VND)',
    barmode='stack',
    xaxis=dict(autorange='reversed'),  # Đảo ngược trục X
    # Độ cao của biểu đồ
)
...
for keyword in keywords:
    index = df_sheet1[df_sheet1.iloc[:, 0].str.contains(keyword, na=False)].index
    if len(index) > 0:
        nguon_von[keyword] = df_sheet1.iloc[index[0], 1:].dropna().values/1000000000

# Chuyển dữ liệu thành DataFrame
combined_df = pd.DataFrame(nguon_von, index=df_sheet1.columns[1:len(list(tai_san.values())[0])+1])
...
# Cập nhật layout của biểu đồ
fig11.update_layout(
    title='Biểu đồ cột chồng về cơ cấu VCSH',
    xaxis_title='Thời gian',
    yaxis_title='Giá trị  (Tỷ VND)',
    barmode='stack',
    xaxis=dict(autorange='reversed'),  # Đảo ngược trục X
...
for keyword in keywords:
    index = df_sheet1[df_sheet1.iloc[:, 0].str.contains(keyword, na=False)].index
    if len(index) > 0:
        von[keyword] = df_sheet1.iloc[index[0], 1:].dropna().values/1000000000

# Chuyển dữ liệu thành DataFrame
combined_df = pd.DataFrame(von, index=df_sheet1.columns[1:len(list(tai_san.values())[0])+1])
...
# Cập nhật layout của biểu đồ
fig12.update_layout(
    title='Biểu đồ cột chồng về cơ cấu vốn',
    xaxis_title='Thời gian',
    yaxis_title='Giá trị  (Tỷ VND)',
    barmode='stack',
    xaxis=dict(autorange='reversed'),  # Đảo ngược trục X
...
for keyword in keywords:
    index = df_sheet1[df_sheet1.iloc[:, 0].str.contains(keyword, na=False)].index
    if len(index) > 0:
        vay_nganh[keyword] = df_sheet1.iloc[index[0], 1:].dropna().values / 1000000000

# Xác định chiều dài của chỉ mục
index_length = len(list(vay_nganh.values())[0])

# Chuyển dữ liệu thành DataFrame
combined_df = pd.DataFrame(vay_nganh, index=df_sheet1.columns[1:index_length+1])
...
# Cập nhật layout của biểu đồ
fig13.update_layout(
    title='Biểu đồ cột chồng về khoản cho vay',
    xaxis_title='Thời gian',
    yaxis_title='Giá trị (Tỷ VND)',
    barmode='stack',
    xaxis=dict(autorange='reversed'),  # Đảo ngược trục X
    width=1000,  # Độ rộng của biểu đồ
    height=600   # Độ cao của biểu đồ
)
#############################
# Đọc dữ liệu từ file Excel (giả sử df_sheet1 đã được đọc)
# df_sheet1 = pd.read_excel('path_to_your_file.xlsx')
...
for keyword in keywords:
    index = df_sheet1[df_sheet1.iloc[:, 0].str.contains(keyword, na=False)].index
    if len(index) > 0:
        thoigianvay[keyword] = df_sheet1.iloc[index[0], 1:].dropna().values / 1000000000

# Kiểm tra nếu không có dữ liệu, tránh lỗi index
if thoigianvay:
    # Xác định chiều dài của chỉ mục
    index_length = len(list(thoigianvay.values())[0])

    # Chuyển dữ liệu thành DataFrame
    combined_df = pd.DataFrame(thoigianvay, index=df_sheet1.columns[1:index_length+1])

    # Vẽ biểu đồ cho phân loại thời gian vay
    fig14 = go.Figure()
    for column in combined_df.columns:
...
Tangtruongchovay__index = df_sheet1[df_sheet1.iloc[:, 0].str.contains('Tăng trưởng cho vay khách hàng', na=False)].index[0]
Tangtruongchovay__data = df_sheet1.iloc[Tangtruongchovay__index, 1:].dropna()*100

# Tạo DataFrame cho biểu đồ tăng trưởng cho vay
combined_df_growth = pd.DataFrame({
    'Quý': df_sheet1.columns[1:len(Tangtruongchovay__data) + 1],
    'Tăng trưởng cho vay': Tangtruongchovay__data.values,
})

# Thêm trace cho biểu đồ tăng trưởng cho vay
fig14.add_trace(go.Scatter(
    x=combined_df_growth['Quý'], 
    y=combined_df_growth['Tăng trưởng cho vay'], 
    name='Tăng trưởng cho vay', 
    mode='lines+markers+text',
    line_shape='spline',
    text=combined_df_growth['Tăng trưởng cho vay'],
...
# Cập nhật layout của biểu đồ
fig14.update_layout(
        title='Biểu đồ Tăng trưởng cho vay khách hàng và Phân loại thời gian vay',
        xaxis_title='Thời gian',
        yaxis=dict(
            title='Giá trị (Tỷ VND)',  # Trục Y1
            showgrid=False,
        ),
        yaxis2=dict(
            title='Tăng trưởng (%)',  # Trục Y2
            overlaying='y',
            side='right',
            showgrid=False,
        ),
        ...
        width=1000,  # Độ rộng của biểu đồ
        height=600,
        barmode='stack'   # Độ cao của biểu đồ
    )
...
# Lấy dữ liệu cho "Tăng trưởng cho vay khá
